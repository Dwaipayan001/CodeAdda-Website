...
import {
  ArrowRight, Bot, BrainCircuit, Check, Code2, Database, MapPin,
  Menu, Network, Play, Sparkles, Terminal, X, Zap,
} from 'lucide-react';
  ...
  return (
    <main>
      ...
      <header className="site-header">
        ...
        <a className="nav-cta" href="#enrol">Book a free demo <ArrowRight size={16} /></a>
        ...
        {menuOpen && <nav className="mobile-nav" aria-label="Mobile navigation">
          <a href="#programs" onClick={() => setMenuOpen(false)}>Programs</a><a href="#agentic-lab" onClick={() => setMenuOpen(false)}>Agentic AI Lab</a><a href="#method" onClick={() => setMenuOpen(false)}>How it works</a><a href="#faq" onClick={() => setMenuOpen(false)}>FAQ</a><a href="#enrol" onClick={() => setMenuOpen(false)}>Book a free demo</a>
        </nav>}
      </header>
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-copy">
          <div className="eyebrow"><span className="pulse" /> Kolkata&apos;s hands-on AI learning lab</div>
          <h1>Don&apos;t just use AI.<br /><em>Build what comes next.</em></h1>
          <p className="hero-lede">Live, project-based <strong>Python, Agentic AI, Machine Learning and Data Science coaching</strong> for curious school and college students in Kolkata.</p>
          <div className="hero-actions"><a className="primary-button" href="#enrol">Start with a free class <ArrowRight size={18} /></a><a className="text-button" href="#programs"><Play size={17} fill="currentColor" /> Explore programs</a></div>
          <div className="trust-row"><div className="avatar-stack" aria-hidden="true"><span>PY</span><span>AI</span><span>ML</span></div><div><strong>Learn by building</strong><small>Small batches · Live mentors · Real projects</small></div></div>
        </div>

        <div className="agent-console" id="agentic-lab">
          <div className="console-topbar"><div className="window-dots" aria-hidden="true"><span /><span /><span /></div><span>AI_LEARNING_AGENT.py</span><div className="online"><i /> ONLINE</div></div>
          <Tabs defaultValue="learn" className="console-tabs">
            <TabsList className="console-tab-list"><TabsTrigger value="learn">01 Learn</TabsTrigger><TabsTrigger value="build">02 Build</TabsTrigger><TabsTrigger value="launch">03 Launch</TabsTrigger></TabsList>
            {(Object.keys(terminalLines) as Array<keyof typeof terminalLines>).map((key) => <TabsContent value={key} key={key} className="console-panel">
              <div className="agent-orbit" aria-hidden="true"><div className="orbit orbit-one"><span /></div><div className="orbit orbit-two"><span /></div><div className="agent-core"><Bot size={34} /><i /></div></div>
              <div className="code-block">{terminalLines[key].map((line, index) => <div className={`code-line ${line.type}`} key={line.text}><span>{String(index + 1).padStart(2, '0')}</span>{line.type === 'prompt' && <b>›</b>}{line.text}</div>)}<span className="cursor" aria-hidden="true" /></div>
            </TabsContent>)}
          </Tabs>
          <div className="console-footer"><span><Zap size={14} /> AGENT MODE</span><span>Python 3.12</span><span className="console-location"><MapPin size={13} /> Kolkata</span></div>
        </div>
      </section>

      <div className="signal-strip" aria-label="Course topics"><span>PYTHON</span><i /><span>AGENTIC AI</span><i /><span>MACHINE LEARNING</span><i /><span>DATA SCIENCE</span><i /><span>GENERATIVE AI</span></div>
      ...
      <section className="agentic-feature" id="method"><div className="section agentic-inner">
        <div className="agentic-copy"><span className="kicker lime-text">// THE AGENTIC ADVANTAGE</span><h2>Learn with AI.<br />Learn to <em>lead AI.</em></h2><p>Our students don&apos;t stop at prompts. They learn how AI agents reason, call tools, work together and solve multi-step problems.</p>
          <ul><li><span><Check /></span><div><strong>Mentor + AI feedback loop</strong><small>Human judgment and instant iteration in every session.</small></div></li><li><span><Check /></span><div><strong>Build your own agent team</strong><small>Create research, study and automation agents with Python.</small></div></li><li><span><Check /></span><div><strong>Portfolio, not just certificates</strong><small>Graduate with demos that prove what you can do.</small></div></li></ul>
          <a className="outline-button" href="#enrol">See the Agentic AI curriculum <ArrowRight size={17} /></a>
        </div>
        <div className="workflow-card"><div className="workflow-title"><Network size={18} /><span>multi_agent_workflow</span><i>RUNNING</i></div><div className="workflow-canvas"><div className="connector c1" /><div className="connector c2" /><div className="connector c3" />
          <div className="node user-node"><span>YOU</span><strong>Your idea</strong><small>“Research my topic”</small></div><div className="node orchestrator-node"><span><Sparkles size={16} /> ORCHESTRATOR</span><strong>Plans the mission</strong><small>Breaks work into steps</small></div>
          <div className="worker-row"><div className="node worker-node"><Bot size={17} /><strong>Research</strong><small>Finds sources</small></div><div className="node worker-node"><Code2 size={17} /><strong>Builder</strong><small>Creates output</small></div><div className="node worker-node"><BrainCircuit size={17} /><strong>Critic</strong><small>Checks quality</small></div></div><div className="output-chip"><Check size={14} /> Task complete · 3 agents collaborated</div>
        </div></div>
      </div></section>

      <section className="section outcomes-section"><span className="kicker">// MADE FOR KOLKATA&apos;S NEXT GENERATION</span><div className="outcomes-grid"><div className="outcome-lead"><h2>Small batches.<br />Big <em>builds.</em></h2><p>Designed around school schedules, college goals and the confidence to create independently.</p></div><div className="metric"><strong>1:8</strong><span>mentor to student<br />batch ratio</span></div><div className="metric"><strong>80%</strong><span>hands-on building<br />in every track</span></div><div className="metric"><strong>12+</strong><span>portfolio projects<br />on advanced paths</span></div></div>
        <div className="local-banner"><MapPin /><div><strong>Rooted in Kolkata. Ready for the world.</strong><span>Hybrid classes for students across Salt Lake, New Town, Garia, Behala and beyond.</span></div><span className="bangla">কোড করো। ভবিষ্যৎ গড়ো।</span></div>
      </section>

      <section className="section faq-section" id="faq"><div><span className="kicker">// QUESTIONS, ANSWERED</span><h2>Before you<br />start building.</h2><p>Still unsure which path fits? A mentor will help you choose during the free demo.</p></div>
